...
import { AiReviewMarkdown } from "@/modules/pull-requests/components/ai-review-markdown";
import { getPullRequestById } from "@/modules/pull-requests/server/get-pull-request";
import type { PullRequestStatus } from "@/modules/pull-requests/types";
import {
  PR_STATUS_LABELS,
  getPrStatusTone,
...
function ReviewBody({
  ...
  if (!review) {
    if (status === "reviewed") {
      return (
        <p className='text-sm text-muted-foreground'>
          No review was generated for this pull request (e.g. no meaningful code changes were found).
        </p>
      );
    }
    return (
