...
import type {
  PullRequestItem,
  RepoPullRequests,
} from "@/modules/pull-requests/types";
import { statusBadge } from "@/modules/dashboard/lib/status-style";
import { AiReviewMarkdown } from "./ai-review-markdown";
import {
  PR_STATUS_LABELS,
  getPrStatusTone,
} from "@/modules/pull-requests/utils/status";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
function PullRequestMeta({ pullRequest }: { pullRequest: PullRequestItem }) {
  ...
  return (
    <div className='flex flex-wrap items-center gap-x-3 gap-y-2 text-xs text-muted-foreground mt-1 mb-2'>
      <Badge variant="outline" className="font-normal text-muted-foreground bg-muted/30 gap-1.5 px-2.5 py-0.5 border-border/50">
        <UserIcon className='size-3' />
        {pullRequest.authorLogin ?? "unknown"}
      </Badge>
      <Badge variant="secondary" className="font-normal text-blue-600 bg-blue-500/10 hover:bg-blue-500/20 dark:text-blue-400 dark:bg-blue-500/20 gap-1.5 px-2.5 py-0.5">
        <GitBranchIcon className='size-3' />
        {pullRequest.baseBranch}
      </Badge>
      <span className="flex items-center gap-2 ml-1 text-muted-foreground/80">
...
function AiReviewAccordion({ pullRequest }: { pullRequest: PullRequestItem }) {
  ...
  if (!pullRequest.reviewComment) {
    if (pullRequest.status === "reviewed") {
      return (
        <p className='text-xs text-muted-foreground'>
          No review was generated (no meaningful code changes found).
        </p>
      );
    }
    return (
