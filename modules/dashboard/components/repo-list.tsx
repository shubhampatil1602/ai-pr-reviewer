...
import { githubReposInfiniteQuery } from "@/modules/github/lib/repos-query";
...
import SyncRepoButton from "@/modules/repo-sync/components/sync-repo-button";
...
export function RepoList() {
  ...
  const {
    ...
    refetchInterval: (query) => {
      const hasSyncing = query.state.data?.pages.some((page) =>
        page.repos.some(
          (repo: DashboardRepo) =>
      ...
      <TableRow>
        <TableCell colSpan={7} className='h-14 text-center'>
          <div className='flex items-center justify-center gap-2 text-muted-foreground'>
            <Loader className='size-4 animate-spin' /> Loading repositories
          </div>
        </TableCell>
      </TableRow>
    );
